...
async function loadObjectProperties(root, threadActorID) {
  const {
    utils
  } = _index.default.objectInspector;
...
function hasBreakpoint(location) {
  return !!breakpoints[(0, _breakpoint.makePendingLocationId)(location)];
}
...
async function setBreakpoint(location, options) {
  const breakpoint = breakpoints[(0, _breakpoint.makePendingLocationId)(location)];

  if (breakpoint && JSON.stringify(breakpoint.options) == JSON.stringify(options)) {
    return null;
  }

  breakpoints[(0, _breakpoint.makePendingLocationId)(location)] = {
    location,
    options
...
async function removeBreakpoint(location) {
  delete breakpoints[(0, _breakpoint.makePendingLocationId)(location)];
  const hasWatcherSupport = commands.targetCommand.hasTargetWatcherSupport();
