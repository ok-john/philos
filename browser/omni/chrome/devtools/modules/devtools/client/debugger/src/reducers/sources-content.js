...
function update(state = initialSourcesContentState(), action) {
  switch (action.type) {
    ...
    case "NAVIGATE":
      return { ...initialSourcesContentState(),
        epoch: state.epoch + 1
      };
  }
...
function updateSourceTextContent(state, action) {
  ...
  if (action.sourceId && action.sourceActorId) {
    throw new Error("Both the source id and the source actor should not exist at the same time");
  }
  ...
  if (action.sourceActorId) {
    state.mutableGeneratedSourceTextContentMap.set(action.sourceActorId, content);
  }
