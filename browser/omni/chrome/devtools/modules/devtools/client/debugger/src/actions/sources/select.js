...
function selectSource(cx, sourceId, sourceActorId, location = {}) {
  return async ({
    dispatch
  }) => {
...
function selectLocation(cx, location, {
  ...
  return async ({
    ...
    if (keepContext && selectedSource && selectedSource.isOriginal != (0, _index.isOriginalId)(location.sourceId)) {
      ...
      location = await (0, _sourceMaps.getRelatedMapLocation)(getState(), sourceMapLoader, location);
    ...
    if (!location.sourceActorId) {
      ...
      sourceActor = (0, _selectors.getSourceActor)(getState(), location.sourceActorId);
    }
...
function jumpToMappedLocation(cx, location) {
  return async function ({
    ...
    const pairedLocation = await (0, _sourceMaps.getRelatedMapLocation)(getState(), sourceMapLoader, location);
