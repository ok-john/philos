...
function initialSourcesTreeState() {
  return {
    ...
    projectDirectoryRootName: _prefs.prefs.projectDirectoryRootName,
...
function update(state = initialSourcesTreeState(), action) {
  switch (action.type) {
    case "ADD_ORIGINAL_SOURCES":
      {
        ...
        const validOriginalSources = action.originalSources.filter(source => isSourceVisibleInSourceTree(source, state.chromeAndExtensionsEnabled, state.isWebExtension));
    ...
    case "INSERT_SOURCE_ACTORS":
      {
        ...
        const newSourceActors = action.sourceActors.filter(sourceActor => isSourceVisibleInSourceTree(sourceActor.sourceObject, state.chromeAndExtensionsEnabled, state.isWebExtension));
    ...
    case "NAVIGATE":
      state = initialSourcesTreeState(); // WebExtension is being special. A navigation is notified when reloading the add-on,
...
function addThread(state, thread) {
  const threadActorID = thread.actor; // When processing the top level target,
...
function updateProjectDirectoryRoot(state, root, name) {
  ...
  if (!root || root.startsWith("top-level")) {
    ...
    _prefs.prefs.projectDirectoryRootName = name;
  }
...
function isSourceVisibleInSourceTree(source, chromeAndExtensionsEnabled, debuggeeIsWebExtension) {
  ...
  !source.isExtension || chromeAndExtensionsEnabled || debuggeeIsWebExtension);
}
...
function addSource(threadItems, source, sourceActor) {
  ...
  if (!groupItem) {
    groupItem = createGroupTreeItem(group, threadItem, source); // Copy children in order to force updating react in case we picked
    // this directory as a project root

    threadItem.children = [...threadItem.children, groupItem]; // As we add a new item, re-sort the groups in this thread
  ...
  const directoryItem = addOrGetParentDirectory(groupItem, parentPath); // Check if a previous source actor registered this source.
  ...
  directoryItem.children = [...directoryItem.children, sourceItem]; // Re-sort the items in this directory
...
function sortItems(a, b) {
  if (a.type == "directory" && b.type == "source") {
    ...
    return a.source.displayURL.filename.localeCompare(b.source.displayURL.filename);
  }
...
function sortThreadItems(a, b) {
  ...
  if (a.thread.targetType == "process" && b.thread.targetType == "frame") {
    return -1;
  } else if (a.thread.targetType == "frame" && b.thread.targetType == "process") {
    return 1;
  } // And we display the worker targets last.


  if (a.thread.targetType.endsWith("worker") && !b.thread.targetType.endsWith("worker")) {
    return 1;
  } else if (!a.thread.targetType.endsWith("worker") && b.thread.targetType.endsWith("worker")) {
    return -1;
...
function addOrGetParentDirectory(groupItem, path) {
  ...
  const parentPath = path.substring(0, path.lastIndexOf("/"));
  const parentDirectory = addOrGetParentDirectory(groupItem, parentPath); // We can now create the new Directory Item and register it in its parent Item.

  const directory = createDirectoryTreeItem(path, parentDirectory); // Copy children in order to force updating react in case we picked
  // this directory as a project root

  parentDirectory.children = [...parentDirectory.children, directory]; // Re-sort the items in this directory

  parentDirectory.children.sort(sortItems); // Also maintain the list of all group items,
