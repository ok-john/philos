...
function getColumnBreakpoints(positions, breakpoints, viewport, selectedSource, selectedSourceTextContent) {
  ...
  positions = filterInLine(positions, selectedSource, selectedSourceTextContent);
...
const getVisibleBreakpointPositions = (0, _reselect.createSelector)(_index.getSelectedSource, _index.getBreakpointPositions, (source, positions) => {
  if (!source) {
    return [];
  }
...
function getFirstBreakpointPosition(state, {
  ...
  const positions = (0, _index.getBreakpointPositionsForSource)(state, sourceId);
  const source = (0, _index.getSource)(state, sourceId);
