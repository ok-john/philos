...
exports.setExceptionPreview = setExceptionPreview;
...
function setPreview(cx, expression, location, tokenPos, cursorPos, target) {
  return async ({
    ...
    const properties = await client.loadObjectProperties(root, thread); // The first time a popup is rendered, the mouse should be hovered
    // on the token. If it happens to be hovered on whitespace, it should
    // not render anything

    if (!target.matches(":hover") && !(0, _environment.isNodeTest)()) {
      return;
...
function setExceptionPreview(cx, target, tokenPos, codeMirror) {
  return async ({
    dispatch,
    getState
  }) => {
