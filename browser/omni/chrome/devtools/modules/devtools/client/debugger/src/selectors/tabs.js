...
const getSourcesForTabs = (0, _reselect.createSelector)(_sources.getSourcesMap, getSourceTabs, (sourcesMap, sourceTabs) => {
  return sourceTabs.map(tab => sourcesMap.get(tab.sourceId)).filter(source => source);
}, {
...
function getNewSelectedSourceId(state, tabList) {
  ...
  const matchingTab = availableTabs.find(tab => (0, _tabs.isSimilarTab)(tab, selectedTab.url, (0, _index.isOriginalId)(selectedLocation.sourceId)));
