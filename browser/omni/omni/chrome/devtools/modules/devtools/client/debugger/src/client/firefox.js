...
async function onConnect(_commands, _resourceCommand, _actions, store) {
  ...
  await commands.threadConfigurationCommand.updateConfiguration(options); // Select the top level target by default

  await actions.selectThread((0, _selectors.getContext)(store.getState()), targetFront.threadFront.actor);
...
function onDocumentEventAvailable(events) {
  for (const event of events) {
    ...
    if (!event.targetFront.isTopLevel) continue; // The debugger doesn't support the iframe dropdown.
