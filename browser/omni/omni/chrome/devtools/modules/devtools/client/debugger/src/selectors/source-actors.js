...
function getSourceActorBreakableLines(state, sourceActorId) {
  return (0, _asyncValue.asSettled)(state.sourceActors.mutableBreakableLines.get(sourceActorId));
...
function getBreakableLinesForSourceActors(state, sourceActorIDs, isHTML) {
  const allBreakableLines = [];

  for (const sourceActorId of sourceActorIDs) {
    const breakableLines = state.sourceActors.mutableBreakableLines.get(sourceActorId);
