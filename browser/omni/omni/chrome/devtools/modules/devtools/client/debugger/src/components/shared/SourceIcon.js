...
var _default = (0, _connect.connect)((state, props) => {
  ...
  const isBlackBoxed = (0, _selectors.isSourceBlackBoxed)(state, source); // For the tab icon, we don't want to show the pretty icon for the non-pretty tab

  const hasMatchingPrettyTab = !forTab && (0, _selectors.hasPrettyTab)(state, source.url); // This is the key function that will compute the icon type,
  // In addition to the "modifier" implemented by each callsite.

  const iconClass = (0, _source.getSourceClassnames)(source, symbols, isBlackBoxed, hasMatchingPrettyTab);
  return {
    iconClass
  };
})(SourceIcon);
