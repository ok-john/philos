...
function searchSources(cx, query) {
  ...
  const search = async ({
    ...
    const validSources = (0, _selectors.getSourceList)(getState()).filter(source => !(0, _selectors.isSourceBlackBoxed)(getState(), source)); // Sort original entries first so that search results are more useful.
...
function searchSource(cx, source, sourceActor, query) {
  return async ({
    ...
    if (content && (0, _asyncValue.isFulfilled)(content) && content.value.type === "text") {
      matches = await searchWorker.findSourceMatches(source.id, content.value, query);
    }
