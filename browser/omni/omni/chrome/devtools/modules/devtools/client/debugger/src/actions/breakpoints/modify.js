...
async function clientSetBreakpoint(client, cx, {
  ...
  const shouldMapBreakpointExpressions = (0, _selectors.isMapScopesEnabled)(getState()) && (0, _selectors.getLocationSource)(getState(), breakpoint.location).isOriginal && (breakpoint.options.logValue || breakpoint.options.condition);
...
function addBreakpoint(cx, initialLocation, options = {}, disabled, shouldCancel = () => false) {
  return async thunkArgs => {
    ...
    const initialSource = (0, _selectors.getLocationSource)(getState(), initialLocation);
    await dispatch((0, _breakpointPositions.setBreakpointPositions)({
      cx,
      sourceId: initialSource.id,
      line
    }));
    const position = column ? (0, _selectors.getBreakpointPositionsForLocation)(getState(), initialLocation) : (0, _selectors.getFirstBreakpointPosition)(getState(), initialLocation); // No position is found if the `initialLocation` is on a non-breakable line or
    ...
    const generatedSource = (0, _selectors.getLocationSource)(getState(), generatedLocation);

    if (!source || !generatedSource) {
      return null;
    }

    const originalContent = (0, _selectors.getSettledSourceTextContent)(getState(), location);
    const originalText = (0, _source.getTextAtPosition)(source.id, originalContent, location);
    const content = (0, _selectors.getSettledSourceTextContent)(getState(), generatedLocation);
