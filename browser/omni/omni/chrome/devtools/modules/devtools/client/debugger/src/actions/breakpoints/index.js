...
function updateBreakpointsForNewPrettyPrintedSource(cx, sourceId) {
  return async ({
    dispatch,
    getState,
    sourceMapLoader
    ...
    const breakpoints = (0, _selectors.getBreakpointsForSource)(getState(), sourceId); // Remap the breakpoints with the original location information from
    // the pretty-printed source.

    const newBreakpoints = await Promise.all(breakpoints.map(async breakpoint => {
      const location = await sourceMapLoader.getOriginalLocation(breakpoint.generatedLocation);
      return { ...breakpoint,
        location
      };
