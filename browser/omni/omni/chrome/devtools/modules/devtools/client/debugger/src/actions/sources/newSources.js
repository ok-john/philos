...
function checkPendingBreakpoints(cx, sourceId, sourceActorId) {
  return async ({
    ...
    const sourceActor = (0, _selectors.getSourceActor)(getState(), sourceActorId); // load the source text if there is a pending breakpoint for it
...
function newGeneratedSources(sourceResources) {
  return async ({
    ...
    for (const newSourceActor of newSourceActors) {
      ...
      if ((0, _source.isInlineScript)(newSourceActor) && (0, _selectors.getSourceTextContent)(getState(), location) != null) {
